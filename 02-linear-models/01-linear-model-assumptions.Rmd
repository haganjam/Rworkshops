...
We can now fit a linear regression model to these sample data and try to estimate the population-level parameters that we simulated ($\beta_0$ and $\beta_1$). A linear regression model takes the following form:

$$
y_i = b_0 + b_1x_i + e_i
$$

Here, the $b_0$ and $b_1$ are estimates of the population-level parameters ($\beta_0$ and $\beta_1$) and the $e_i$ values are the model residuals i.e. the difference between the observed $y_i$ and the predicted value which is $\hat{y_i}$
...
As described previously, the $b_0$ and $b_1$ are estimates of the population-level parameters ($\beta_0$ and $\beta_1$) and the $e_i$ values are the model residuals i.e. the difference between the observed $y_i$ and the predicted value which is $\hat{y_i}$
